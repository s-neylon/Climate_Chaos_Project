...
Some years where I calculate there was hurricane damage, there isn't this damage in Deryugina.
  ...
  -> SOLUTION: I am going with the Deryugina years to determine when a hurricane is!
  -> The file "SHELDUS_Dery_SHELDUS-hurr-definition" is where the code for the SHELDUS definition of a hurricane is.
...
The variables below are for when there is a SHELDUS-defined hurricane. I started to work on the Deryugina variables, but realized that not all SHELDUS hurricanes have wind speeds of 74mph.

In the main file, I am going to only define hurricanes in the Deryugina way, but want to keep this code here in case I need it.
...
'post_treat' is an indicator for the post-treatment period for each treated county.

### SHELDUS

Any Hurricane Damage (serious threshold not used), between 1979-2002.

```{r}

HrSHLD_dery_df <- HrSHLD_dery_df %>%
  # Serious hurricane damage
  mutate(any_hurrDMG = if_else(PropertyDmg_Adj2022 > 0 & year >= 1979 & year <= 2002, 1, 0)) %>%
  # Group by county
  group_by(FIPS_5) %>%
  # Sort by year within each county
  arrange(FIPS_5, year) %>%
  # Create the treatment variable
  mutate(post_treat_SHLD = cummax(any_hurrDMG)) %>%
  ungroup()
...
HrSHLD_dery_df <- HrSHLD_dery_df %>%
  ...
  arrange(FIPS_5, year) %>%
  mutate(
    first_hurr_year_SHLD = ifelse(any(any_hurrDMG == 1), min(year[any_hurrDMG == 1]), NA_real_),
...
# Full Data
write_csv(HrSHLD_dery_df, here("data/SHELDUS/Deryugina_hurr_SHELDUS.csv"))
...
hs_HrSHLD_dery_df <- HrSHLD_dery_df %>% 
...
write_csv(hs_HrSHLD_dery_df, here("data/SHELDUS/hs_Deryugina_hurr_SHELDUS.csv"))
