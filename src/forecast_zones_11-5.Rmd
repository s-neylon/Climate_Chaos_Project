...
nws_detail_22 <- nws_detail_22 %>% mutate(
  ...
  STATE_ZONE = str_c(Postal.x, full_FIPS)
)
...
## Sample Set
Working with smaller sample - New Mexico has zones in multiple counties, so a good place to start.

```{r}

NM_nws <- nws_detail_22 %>% filter(STATE_FIPS==35)
...
hurr_nws <- nws_detail_22 %>% filter(EVENT_TYPE == "Hurricane")
...
FL_zone <- zone_county %>% filter(STATE == "FL")

tstFL_zone <- FL_zone %>% left_join()
...
hr_join <- inner_join(hurr_nws, zone_county, join_by(STATE_ZONE == STATE_ZONE))
...
NM_join <- inner_join(NM_nws, zone_county, join_by(STATE_ZONE == STATE_ZONE))
...
Okay, so the zone_county list is a full list of all the zone-county combinations, so I should be using IT as the left in the join.
